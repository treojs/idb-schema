 ...
 *
 * @param {String} name
 * @param {Object} [opts] { key: undefined, increment: undefined }
 * @return {Schema}
 */
...
Schema.prototype.addStore = function(name, opts) {
  ...
  var store = {
    name: name,
    indexes: {},
    keyPath: opts.key || opts.keyPath || undefined,
    autoIncrement: opts.increment || opts.autoIncrement || undefined
  };
...
Schema.prototype.callback = function() {
  var versions = values(this._versions)
 ...
 * It creates a depp clone of `this._stores` object and transform it to array.
 *
 * @return {Array}
 */

Schema.prototype.stores = function() {
  var stores = JSON.parse(JSON.stringify(this._stores)); // clone
  return values(stores).map(function(store) {
    store.indexes = values(store.indexes);
    return store;
  });
};
